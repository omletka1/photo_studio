    ...
    <header class="page-header">
        <div class="header-left">
            <?= Html::a('
                ...
                Назад
            ', ['/admin/submission/index'], ['class' => 'btn-back', 'encode' => false]) ?>
        ...
        <div class="header-actions">
            <?= Html::a('
                ...
                Оценить
            ', ['/admin/jury/rate', 'submission_id' => $work->id], ['class' => 'btn-secondary', 'encode' => false]) ?>
        </div>
    </header>
    ...
    <div class="content-grid">
        ...
        <aside class="content-sidebar">
            ...
            <div class="card">
                ...
                <?= Html::a('Просмотреть профиль', ['/account/view-profile', 'id' => $work->user_id], ['class' => 'link-secondary']) ?>
            </div>
            ...
            <div class="card">
                ...
                <?= Html::a('Перейти к конкурсу', ['/admin/konkurs/view', 'id' => $work->konkurs_id], ['class' => 'link-secondary']) ?>
            </div>
                ...
                <div class="stats-list">
                    ...
                    <div class="stat-row">
                        ...
                        $statusClass = match($work->status) {
                            1 => 'status-gold', 2 => 'status-silver', 3 => 'status-bronze',
                            default => 'status-default'
                        };
                        ?>
                        <span class="stat-value status-badge <?= $statusClass ?>"><?= $statusText ?></span>
                    </div>
                </div>
            </div>
            ...
            <div class="card">
                ...
                <?php $form = \yii\widgets\ActiveForm::begin(['method' => 'post', 'action' => ['update-status', 'id' => $work->id], 'options' => ['class' => 'status-form']]); ?>
                <?= $form->field($work, 'status')->dropDownList([
                    0 => 'Участник', 1 => '1 место', 2 => '2 место', 3 => '3 место',
            ...
            <?php else: ?>
                <?php foreach ($comments as $comment): ?>
                    <?= $this->render('@app/modules/admin/views/jury/_comment', [
                        ...
                        'isVisible' => true  // 🔥 Показываем корневые комментарии
                    ]) ?>
                <?php endforeach; ?>
            <?php endif; ?>
        </div>
...
<style>
    ...
    .comment-item.hidden, .reply-item.hidden { display: none !important; }

    .btn-show-more {
        display: block; width: 100%; padding: 10px; margin-top: 8px;
        background: #f3f4f6; color: #ea580c; border: none; border-radius: 8px;
        font-weight: 500; cursor: pointer; transition: background 0.2s;
        text-align: center;
    }
    .btn-show-more:hover { background: #e5e7eb; }
    ...
    .status-silver { background: #f1f5f9; color: #334155; }
    .status-bronze { background: #ffedd5; color: #7c2d12; }
    ...
    .form-select {
        ...
        font-size: 0.9rem; cursor: pointer; transition: border-color 0.2s ease, box-shadow 0.2s ease;
    }
    .form-select:focus { outline: none; border-color: #8b77b3; box-shadow: 0 0 0 3px rgba(139,119,179,0.15); }

    .link-secondary {
        color: #8b77b3; text-decoration: none; font-size: 0.9rem; font-weight: 500;
        transition: color 0.2s ease; display: inline-block; margin-top: 8px;
    }
    .link-secondary:hover { color: #75639c; text-decoration: underline; }
    ...
    .modal-backdrop {
        ...
        z-index: 1; /* Ниже карточки */
    }

    /* Карточка модалки — выше бэкдропа */
    .modal-card {
        position: relative;
        background: #fff;
        border-radius: 16px;
    ...
    .form-textarea:focus { outline: none; border-color: #8b77b3; box-shadow: 0 0 0 3px rgba(139,119,179,0.15); }
...
<script>
    ...
    // Show more replies
    // Show more replies — ИСПРАВЛЕННАЯ ВЕРСИЯ
    function showMoreReplies(button) {
        const container = button.closest('.replies');
        if (!container) return;

        // 🔥 Ищем элементы с классом .hidden, а не с inline-стилем!
        const hidden = Array.from(container.querySelectorAll('.reply-item.hidden'));

        // Показываем следующие 3
        hidden.slice(0, 3).forEach(el => el.classList.remove('hidden'));

        // Обновляем кнопку
        const remaining = container.querySelectorAll('.reply-item.hidden').length;
        if (remaining <= 0) {
            button.remove();
        } else {
            ...
            form.addEventListener('submit', function(e) {
                ...
                fetch("<?= \yii\helpers\Url::to(['/admin/jury/add-comment']) ?>", {
                    ...
                    .then(data => {
                        ...
                        // Insert new comment
// 🔥 Вставка нового комментария — НАДЁЖНАЯ ВЕРСИЯ
                        const temp = document.createElement('div');
                        temp.innerHTML = data.html?.trim() || '';
                        const newComment = temp.firstElementChild;

// 🔥 Проверка: если HTML пустой или невалидный
                        if (!newComment) {
                            console.error('❌ data.html is empty or invalid:', data.html);
                            alert('Ошибка: сервер вернул пустой комментарий');
                            // Но всё равно закрываем модалку и сбрасываем форму
                            this.reset();
                            clearParent(submissionId);
                            closeCommentModal(submissionId);
                            return;
                        }

// 🔥 Убираем класс hidden, если он есть (новый комментарий должен быть виден!)
                        newComment.classList.remove('hidden');
                        newComment.style.display = '';

                        if (data.parent_id) {
                            // 🔥 Это ОТВЕТ: ищем родительский комментарий
                            const parent = document.querySelector(`[data-id="${data.parent_id}"]`);

                            if (parent) {
                                // 🔥 Ищем или создаём контейнер .replies
                                let repliesContainer = parent.querySelector('.replies');
                                if (!repliesContainer) {
                                    repliesContainer = document.createElement('div');
                                    repliesContainer.className = 'replies mt-3 space-y-2';
                                    // Вставляем после текста комментария, но до кнопок
                                    const commentContent = parent.querySelector('.flex-1');
                                    if (commentContent?.nextElementSibling) {
                                        commentContent.parentElement.insertBefore(repliesContainer, commentContent.nextElementSibling);
                                    } else {
                                ...
                                // 🔥 Обновляем кнопку "Показать ещё", если ответов > 3
                                updateShowMoreButton(repliesContainer, '.reply-item');
                            } else {
                                console.warn('⚠️ Parent comment not found:', data.parent_id);
                                // Фолбэк: добавляем в общий список, если родителя не нашли
                            ...
                            // 🔥 Это НОВЫЙ корневой комментарий: добавляем в начало списка
                            const list = document.getElementById('comment-list-' + submissionId);
                            if (list) {
                                list.querySelector('.comment-empty')?.remove();
                                list.prepend(newComment);
                                ...
                                // 🔥 Обновляем кнопку "Показать ещё" для основного списка
                                updateShowMoreButton(list, '.comment-item');
                            }
                        }

// 🔥 Подсветка нового комментария
                        newComment.style.background = '#f0eaf5';
                        newComment.style.transition = 'background 0.3s ease';
                        setTimeout(() => {
...
// 🔥 Сброс формы и закрытие модалки
    ...
    // 🔥 Показать ещё комментарии (основной список)
    function showMoreComments(button) {
        const container = button.closest('.comment-list');
        if (!container) return;
        const hidden = container.querySelectorAll('.comment-item.hidden');
        Array.from(hidden).slice(0, 5).forEach(el => el.classList.remove('hidden'));
        const remaining = container.querySelectorAll('.comment-item.hidden').length;
        if (remaining <= 0) {
            button.remove();
        } else {
            button.querySelector('span').textContent = remaining;
        }
    }

    // 🔥 Хелпер: обновляет кнопку "Показать ещё" после добавления нового элемента
    function updateShowMoreButton(container, itemSelector) {
        const items = container.querySelectorAll(itemSelector);
        const hidden = container.querySelectorAll(`${itemSelector}.hidden`);
        const visibleCount = items.length - hidden.length;

        let btn = container.querySelector('.btn-show-more, .btn-show-more-replies');

        if (hidden.length > 0) {
            // 🔥 Есть скрытые элементы → показываем/обновляем кнопку
            if (!btn) {
                btn = document.createElement('button');
                btn.type = 'button';
                btn.className = itemSelector === '.reply-item'
                    ? 'btn-show-more-replies text-xs text-orange-600 hover:underline'
                    : 'btn-show-more';
                btn.onclick = function() {
                    itemSelector === '.reply-item' ? showMoreReplies(this) : showMoreComments(this);
                };
                container.appendChild(btn);
            }
            btn.textContent = 'Показать ещё ' + hidden.length;
            btn.style.display = 'inline-block';
        } else if (btn) {
            // 🔥 Всё показано → убираем кнопку
            btn.remove();
        }
    }
</script>