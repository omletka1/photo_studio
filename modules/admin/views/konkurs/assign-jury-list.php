...
$this->title = 'Назначение жюри';
?>

<!-- Убираем лишние обёртки — .main-container уже в main.php -->
<div class="jury-assign-list">

    <header class="page-header">
        <h1 class="page-title">Назначение жюри</h1>
        <p class="page-subtitle">Выберите конкурс для назначения судей</p>
    </header>
    ...
    <?php if (empty($konkurs)): ?>
        <div class="empty-state rv active">
            ...
            <p>Открытые конкурсы не найдены</p>
        </div>
    <?php else: ?>
        <div class="konkurs-list">
            <?php foreach ($konkurs as $index => $k): ?>
                <div class="konkurs-item rv" style="transition-delay: <?= $index * 0.05 ?>s">
                    ...
                    <div class="konkurs-actions">
                        <?= Html::a('
                            ...
                            Назначить
                        ', ['/admin/konkurs/assign-jury', 'id' => $k->id], ['class' => 'btn-assign', 'encode' => false]) ?>
                    </div>
                </div>
            <?php endforeach; ?>
        </div>
...
<style>
    ...
    .konkurs-item.active { opacity: 1; transform: translateY(0); }
    .konkurs-item:hover {
    ...
    @media (max-width: 640px) {
        .konkurs-item { flex-direction: column; align-items: flex-start; }
